// https://github.com/Extrimis/Win32-Acrylic-Effect

// MIT License
// Copyright (c) 2021 Selastin George

#include "acrylic_compositor.h"
#include "win32_check.h"

namespace Acrylic {
    std::unique_ptr<AcrylicCompositor> AcrylicCompositor::Create(unsigned long buildVersion) {
        ...
        auto DwmSetWindowCompositionAttribute = (SetWindowCompositionAttribute)GetProcAddress(user32, "SetWindowCompositionAttribute");
        auto DwmCreateSharedThumbnailVisual = (DwmpCreateSharedThumbnailVisual)GetProcAddress(dwmapi, MAKEINTRESOURCEA(147));
        auto DwmCreateSharedMultiWindowVisual = (DwmpCreateSharedMultiWindowVisual)GetProcAddress(dwmapi, MAKEINTRESOURCEA(163));
        auto DwmUpdateSharedMultiWindowVisual = (DwmpUpdateSharedMultiWindowVisual)GetProcAddress(dwmapi, MAKEINTRESOURCEA(164));
        auto DwmCreateSharedVirtualDesktopVisual = (DwmpCreateSharedVirtualDesktopVisual)GetProcAddress(dwmapi, MAKEINTRESOURCEA(163));
        auto DwmUpdateSharedVirtualDesktopVisual = (DwmpUpdateSharedVirtualDesktopVisual)GetProcAddress(dwmapi, MAKEINTRESOURCEA(164));

        return std::make_unique<AcrylicCompositor>(buildVersion, DwmCreateSharedThumbnailVisual,
                                                     DwmCreateSharedMultiWindowVisual, DwmUpdateSharedMultiWindowVisual,
                                                     DwmCreateSharedVirtualDesktopVisual, DwmUpdateSharedVirtualDesktopVisual,
                                                     DwmSetWindowCompositionAttribute);
    }

    AcrylicCompositor::AcrylicCompositor(unsigned long buildVersion, DwmpCreateSharedThumbnailVisual DwmCreateSharedThumbnailVisual,
        ...
        : build_version(buildVersion), DwmCreateSharedThumbnailVisual(DwmCreateSharedThumbnailVisual),
          DwmCreateSharedMultiWindowVisual(DwmCreateSharedMultiWindowVisual), DwmUpdateSharedMultiWindowVisual(DwmUpdateSharedMultiWindowVisual),
          DwmCreateSharedVirtualDesktopVisual(DwmCreateSharedVirtualDesktopVisual),
          DwmUpdateSharedVirtualDesktopVisual(DwmUpdateSharedVirtualDesktopVisual),
          DwmSetWindowCompositionAttribute(DwmSetWindowCompositionAttribute) {
        check(CreateCompositionDevice());
        check(CreateEffectGraph(dcompDevice3));
    }
    ...
    bool AcrylicCompositor::CreateCompositionDevice() {
        if (D3D11CreateDevice(0, D3D_DRIVER_TYPE_HARDWARE, NULL, D3D11_CREATE_DEVICE_BGRA_SUPPORT, NULL, 0, D3D11_SDK_VERSION,
                              d3d11Device.GetAddressOf(), nullptr, nullptr) != S_OK) {
            return false;
        }
        ...
        if (D2D1CreateFactory(D2D1_FACTORY_TYPE::D2D1_FACTORY_TYPE_SINGLE_THREADED, __uuidof(ID2D1Factory2), (void **)d2dFactory2.GetAddressOf()) !=
            S_OK) {
            return false;
        }
    ...
    bool AcrylicCompositor::CreateBackdrop(HWND hwnd, BackdropSource source) {
        ...
        case BACKDROP_SOURCE_DESKTOP:
            ...
            thumbnail.dwFlags =
                DWM_TNP_SOURCECLIENTAREAONLY | DWM_TNP_VISIBLE | DWM_TNP_RECTDESTINATION | DWM_TNP_RECTSOURCE | DWM_TNP_OPACITY | DWM_TNP_ENABLE3D;
            ...
            if (DwmCreateSharedThumbnailVisual(hwnd, desktopWindow, 2, &thumbnail, dcompDevice.Get(), (void **)desktopWindowVisual.GetAddressOf(),
                                               &desktopThumbnail) != S_OK) {
                return false;
            }
            break;
        case BACKDROP_SOURCE_HOSTBACKDROP:
            if (build_version >= 20000) {
                hr =
                    DwmCreateSharedMultiWindowVisual(hwnd, dcompDevice.Get(), (void **)topLevelWindowVisual.GetAddressOf(), &topLevelWindowThumbnail);
            } else {
                hr = DwmCreateSharedVirtualDesktopVisual(hwnd, dcompDevice.Get(), (void **)topLevelWindowVisual.GetAddressOf(),
                                                         &topLevelWindowThumbnail);
            }
